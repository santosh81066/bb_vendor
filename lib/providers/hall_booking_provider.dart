...
// Updated _getBookingId method
  ...
  Future<int> countUniqueBlockedUsersPerDay(int hallId) async {
    ...
    if (response.statusCode == 200) {
      ...
      final bookings = jsonResponse['data'] as List; // Fixed: Accessing 'data' instead of 'messages'
      ...
      for (var booking in bookings) {
        ...
        final bookingHallId = booking['hall_id'];

        // Filter by hall_id and blocked status
        if (status == 'b' && userId != null && date != null && bookingHallId == hallId) {
